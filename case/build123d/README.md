...
背板上重复了 **4 组 M4 柱位**(旋转 10/20/30/40°):每组是同一 4 孔图案
绕图案质心旋转一个角度,换组重装螺丝,键盘的平面内旋转角就变,home row
的倾斜度随之调整——手完全垂直时 home row 接近水平,手腕前倾时换更大
角度的组。

- 每组的位置由求解器自动排布:横向错开保证任意两柱间距 ≥13 mm,并逐组
  抬升,保证旋转后板角距底座顶面 ≥2.5 mm、底部柱连线距台面 ≥43 mm
  (`--set-angles 10,20,30,40` 可改角度与组数,负值 = 反方向倾斜);
- **5° 步进几何上不可行**:12 mm 的柱在 5° 间隔下同孔柱心距仅 4-5 mm,
  相邻组的柱会融成一体;10° 步进 + 每组横向错位是柱径允许的最小方案;
- 代价:下部背板柱位密集(16 柱),加强肋自动避让后只放得下 2 根,
  交界刚度主要由托肩与埋入承担。
...
1. 选定旋转组的 4 根 M4 柱与 PCB 对位(fit 件含全部 4 组柱位,按你用的
   那组试装螺钉);
